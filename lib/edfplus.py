...
file = pyedflib.EdfReader('Bill.edf')
...
def edf_header(f):
  ...
  # misc
  #h['n_header_bytes'] = header_nbytes = int(f.read(8).decode('utf-8'))
  subtype = f.read(44)[:5]
  h['EDF+'] = subtype in ['EDF+C', 'EDF+D']
  h['contiguous'] = subtype != 'EDF+D'
  h['n_records'] = file.datarecords_in_file
  h['record_length'] = file.datarecord_duration  # in seconds
  nchannels = h['n_channels'] = file.signals_in_file
  ...
  h['physical_min'] = file.getPhysicalMinimum
  h['physical_max'] = file.getPhysicalMaximum
  h['digital_min'] = file.getDigitalMinimum
  h['digital_max'] = file.getDigitalMaximum
  h['prefiltering'] = [f.read(80).strip() for n in channels]
  h['n_samples_per_record'] = file.getNSamples()
...
class BaseEDFReader:
  ...
  def read_header(self):
    ...
    self.dig_min = file.getDigitalMinimum
    self.phys_min = file.getPhysicalMinimum
    pmin = file.getPhysicalMinimum(33)
    pmax = file.getPhysicalMaximum(33)
    dmin = file.getDigitalMinimum(33)
    dmax = file.getDigitalMaximum(33)
    phys_range = pmax - pmin
    dig_range = dmax - dmin
  ...
  def convert_record(self, raw_record):
    ...
    '''
    h = self.header
    dig_min, phys_min, gain = file.getDigitalMinimum, file.getPhysicalMinimum, self.gain
    time = float('nan')
    signals = []
    events = []
    for (i, samples) in enumerate(raw_record):
      if h['label'][i] == EVENT_CHANNEL:
        ...
        print(dig)
        phys = (dig[i] - dig_min(i)) * gain(i) + phys_min(i)
        signals.append(phys)
